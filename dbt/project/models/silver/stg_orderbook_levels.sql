{{ config(unique_key=['product_id', 'side', 'price']) }}
...
with snap as (
    ...
    from {{ source('bronze', 'orderbook_events') }}
    where type = 'snapshot'

    {% if is_incremental() %}
    and ingest_time > (select coalesce(max(event_time), timestamp('1970-01-01')) from {{ this }})
    {% endif %}
